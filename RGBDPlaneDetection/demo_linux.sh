...
RGBD="/Volumes/Work/2019WorkArea/FeatureDetection/Dataset/ZN603A/"
...
OUTPUT="/Volumes/Work/2019WorkArea/FeatureDetection/Dataset/ZN603A/planes-all-frames"
if [ ! -d "$OUTPUT" ]; then
  mkdir $OUTPUT
fi
code="./build/Debug/RGBDPlaneDetection"
...
# done
num=59
for i in  $(seq -f "%06g" 1 $num)
do
	cecho g "Reading frame $i"
  # $code ${RGBD}frame-$i-color.png ${RGBD}frame-$i-depth.png $OUTPUT
  $code -o ${RGBD}frame-$i.color.png ${RGBD}frame-$i.depth.png $OUTPUT
